'use client';

import { Loader, TextInput } from '@mantine/core';
import { Controller } from 'react-hook-form';

/**
 * Renders the main form component for the application.
 *
 * @param {Object} props - The component props.
 * @param {function} props.onClick - The function to call when the "Connect" button is clicked.
 * @param {Object} props.control - The React Hook Form control object.
 * @returns {JSX.Element} The rendered main form component.
 */
export default function MainFormComponent({ onClick, control, loading }) {
  return (
    <div className='bg-form-bg bg-cover h-screen w-full flex justify-center items-center'>
      <div className='w-2/3 md:w-1/2 bg-form-bg-color text-white flex flex-col gap-y-6 py-12 px-8 rounded-3xl border border-[#09CBFB] shadow-[0_2px_4px_-1px_#09CBFB]'>
        <span className='text-2xl 2xl:text-3xl'>
          Provide the keys for database sync
        </span>
        <Controller
          name='sender_url'
          control={control}
          render={({ field }) => (
            <TextInput
              {...field}
              label='Sender url'
              classNames={{
                label: '!text-lg !pb-1',
                input: '!text-lg !h-[3rem] !font-stmono',
              }}
            />
          )}
        />
        <Controller
          name='receiver_url'
          control={control}
          render={({ field }) => (
            <TextInput
              {...field}
              label='Receiver url'
              classNames={{
                label: '!text-lg !pb-1',
                input: '!text-lg !h-[3rem] !font-stmono',
              }}
            />
          )}
        />
        <div className='flex justify-center'>
          <button className='connect' onClick={onClick}>
            <svg
              height='24'
              width='24'
              viewBox='0 0 24 24'
              xmlns='http://www.w3.org/2000/svg'
            >
              <path d='M0 0h24v24H0z' fill='none'></path>
              <path
                d='M5 13c0-5.088 2.903-9.436 7-11.182C16.097 3.564 19 7.912 19 13c0 .823-.076 1.626-.22 2.403l1.94 1.832a.5.5 0 0 1 .095.603l-2.495 4.575a.5.5 0 0 1-.793.114l-2.234-2.234a1 1 0 0 0-.707-.293H9.414a1 1 0 0 0-.707.293l-2.234 2.234a.5.5 0 0 1-.793-.114l-2.495-4.575a.5.5 0 0 1 .095-.603l1.94-1.832C5.077 14.626 5 13.823 5 13zm1.476 6.696l.817-.817A3 3 0 0 1 9.414 18h5.172a3 3 0 0 1 2.121.879l.817.817.982-1.8-1.1-1.04a2 2 0 0 1-.593-1.82c.124-.664.187-1.345.187-2.036 0-3.87-1.995-7.3-5-8.96C8.995 5.7 7 9.13 7 13c0 .691.063 1.372.187 2.037a2 2 0 0 1-.593 1.82l-1.1 1.039.982 1.8zM12 13a2 2 0 1 1 0-4 2 2 0 0 1 0 4z'
                fill='currentColor'
              ></path>
            </svg>
            {loading ? <Loader /> : <span>Connect</span>}
          </button>
        </div>
      </div>
    </div>
  );
}
